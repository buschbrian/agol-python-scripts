...
|---|---|---|
| `assign_polygon_colors.py` | Assigns a `Color_ID` to polygons so no two adjacent polygons share a value (greedy graph coloring over a Polygon Neighbors table) — for cartography where touching parcels or zones need distinct fills. Set the two path variables at the top before running. | ArcGIS Pro's Python (`arcpy`) |
...
| [`utah-road-labels/`](utah-road-labels/) | Labels UGRC's Utah Roads for a 1:30,000 map: vector Interstate, US and Utah beehive shields drawn from the MUTCD/UDOT sign designs, with live route numbers fitted to each shield (Bahnschrift), and Arcade street-name classes keyed on `CARTOCODE` in the Roadgeek FHWA fonts, spaced by Maplex. Optionally packages the result as vector tiles for ArcGIS Online. See its README for setup (Roadgeek fonts) and licence notes. | ArcGIS Pro's Python (`arcpy`) |
